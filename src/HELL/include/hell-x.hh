...
namespace HELLx {
  ...
  extern void SetLLpMode(bool use_LLp=true);
  ...
  private:
    ...
    string datapath_;
    map<int,xTableP*>    xT[2];
    map<int,xTableC*>    xTC[2];
    map<int,xTableCm*>   xTCm[2];
    map<int,xTableCggH*> xTCggH[2];
    ...
    double DeltaCggH(double as, double x, double muFrat, Order matched_to_fixed_order, bool aux=false);
  public:
    HELLxnf(int nf, LogOrder order, string datapath="./data/") : _nf(nf), _order(order), _RCvar(0) { Init(datapath); }
    ~HELLxnf();
    //
    ...
    double deltaMC2axg(double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive F2, axial contribution
    double deltaMC2axq(double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive F2, axial contribution
    double deltaMCLaxg(double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive FL, axial contribution
    double deltaMCLaxq(double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive FL, axial contribution
    ...
    double deltaMC2CCg(double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive F2
    double deltaMC2CCq(double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive F2
    double deltaMCLCCg(double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive FL
    double deltaMCLCCq(double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive FL
    double deltaMC3CCg(double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive F3  (this is for production of heavy quark. Heavy antiquark have an overall opposite sign)
    double deltaMC3CCq(double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive F3  (this is for production of heavy quark. Heavy antiquark have an overall opposite sign)
    // FONLL=SACOT is obtained (for a single quark flavour, e.g. charm) as
    ...
    double deltaCggH   (double as, double x, double muFrat=1, Order matched_to_fixed_order = NLO);  // ggH, gg channel
    double deltaCggHaux(double as, double x, double muFrat=1, Order matched_to_fixed_order = NLO);  // ggH, auxiliary integral for qg and qq channels
  };
  ...
  public:
    ...
    double deltaMC2axg(int nf, double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive F2, axial contribution
    double deltaMC2axq(int nf, double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive F2, axial contribution
    double deltaMCLaxg(int nf, double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive FL, axial contribution
    double deltaMCLaxq(int nf, double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive FL, axial contribution
    // Please note that massless coefficient functions contain a factor of nf in them.
    // FONLL=SACOT is obtained (for a single quark flavour, e.g. charm) as
    //   deltaC2_FONLL = deltaMC2i - deltaKhi      (i=g,q) for F2
    //   deltaCL_FONLL = deltaMCLi                 (i=g,q) for FL
    ...
    //
    // Massive DIS, Charged Current
    double deltaMC2CCg(int nf, double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive F2
    double deltaMC2CCq(int nf, double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive F2
    double deltaMCLCCg(int nf, double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive FL
    double deltaMCLCCq(int nf, double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive FL
    double deltaMC3CCg(int nf, double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive F3  (this is for production of heavy quark. Heavy antiquark have an overall opposite sign)
    double deltaMC3CCq(int nf, double as, double x, double m_Q_ratio, Order matched_to_fixed_order = NLO);  // DIS massive F3  (this is for production of heavy quark. Heavy antiquark have an overall opposite sign)
    // FONLL=SACOT is obtained (for a single quark flavour, e.g. charm) as
    //   deltaC2CC_FONLL = deltaMC2i - deltaKhi/2      (i=g,q) for F2
    //   deltaCLCC_FONLL = deltaMCLi                   (i=g,q) for FL
    //   deltaC2CC_FONLL = deltaMC2i + deltaKhi/2      (i=g,q) for F3  (here the sign is for production of heavy quark. Heavy antiquark have an overall opposite sign)
