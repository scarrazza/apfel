...
\begin{equation}
F(x_\alpha)=C(x_\alpha)\otimes Q(x_\alpha) = \int_{x_\alpha}^1\frac{dy}{y}C(y)Q\left(\frac{x_\alpha}{y}\right)\,,
\end{equation}
where $x_\alpha$ is node of the $x$-space grid of {\tt APFEL}. Now,
using the well-known interpolation formula we can write:
\begin{equation}
\int_{x_\alpha}^1\frac{dy}{y}C(y)Q\left(\frac{x_\alpha}{y}\right) = \sum_{\beta}\underbrace{\left[\int_{x_\alpha}^1\frac{dy}{y}C(y)w_{\beta}^{(k)}\left(\frac{x_\alpha}{y}\right)\right]}_{\Gamma_{\alpha\beta}}Q(x_\beta)\,,
\end{equation}
where $w_{\beta}^{(k)}$ are the usual interpolation functions of
degree $k$. Now suppose that in turn:
\begin{equation}
Q(x_\beta)=P(x_\beta)\otimes f(x_\beta) = \int_{x_\beta}^1\frac{dz}{z}P(z)f\left(\frac{x_\beta}{z}\right)=\sum_{\gamma}\underbrace{\left[\int_{x_\beta}^1\frac{dz}{z}P(z)w_{\gamma}^{(k)}\left(\frac{x_\beta}{z}\right)\right]}_{\Pi_{\beta\gamma}}f(x_\gamma)\,,
\end{equation}
it follows that:
\begin{equation}
F(x_\alpha)=C(x_\alpha)\otimes P(x_\alpha)\otimes f(x_\alpha) =
\sum_{\beta,\gamma} \Gamma_{\alpha\beta}\Pi_{\beta\gamma}f(x_\gamma)\,.
\end{equation}
