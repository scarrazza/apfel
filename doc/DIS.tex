...
\begin{equation}
\frac{K_j}{j(j-1)} = \frac{1}{\langle e_q^2\rangle}\underbrace{\left(\frac{1}{j(j-1)}\sum_{i=1}^je_i^2-\frac{1}{j-1}e_j^2\right)}_{d_j}\,.
\end{equation}
...
\end{equation}
 ...
 so that, up to $\mathcal{O}(\alpha_s^2)$, we have:
\begin{equation}
\begin{array}{ll}
  P_{ij}^{(k)} \rightarrow P_{ij}^{(k)} &\quad
                                          i,j=g,q(\Sigma)\\
  P_{ij}^{(k)} \rightarrow \delta_{ij}P_-^{(k)} &\quad
                                                  i,j=V,V_{3},V_{8},V_{15},V_{24},V_{35}\\
  P_{ij}^{(k)} \rightarrow
  \delta_{ij}P_+^{(k)} &\quad i,j=T_{3},T_{8},V_{15},T_{24},T_{35}
\end{array}
\end{equation}
and:
\begin{equation}
...
\end{array}
\end{equation}
where the $d_j$ are just numerical coefficients that in general can
depend on the number of active flavours $n_f$. In addition:
\begin{equation}
  P_-^{(0)}=P_+^{(0)}=P_{qq}^{(0)}\,,
\end{equation}
and:
...
\begin{equation} 
  ...
  \frac{1}{n_f}\mathcal{C}_+^{(k)}
\end{equation} 
and:
\begin{equation} 
  C_q^{(k)} = C_{\rm PS}^{(k)} + \frac{1}{n_f} C_+^{(k)}
\end{equation} 
