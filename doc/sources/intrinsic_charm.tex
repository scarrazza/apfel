...
\end{equation}
\begin{equation}
F_2^{\rm FF,IC}(x,Q^2)=\left(\sqrt{1+4\frac{m_c^2}{Q^2}}\right)e_c^2xc(\chi)
\end{equation}
\begin{equation}
...
with:
\begin{equation}
\chi = \frac{x}{2}\left(1+\sqrt{1+4\frac{m_c^2}{Q^2}}\right) = \frac{x}{\eta}\,,
\end{equation}
where I have defined:
\begin{equation}\label{Rescaledx}
\eta=\frac{2Q^2}{\Sigma_{+-}+\Delta} = 2\left(1+\sqrt{1+4\frac{m_c^2}{Q^2}}\right)^{-1}=2\left(1+\sqrt{1+4\lambda}\right)^{-1}\,,
\end{equation}
...
and:
\begin{equation}
\chi = \frac{x}{2}\left(1+\sqrt{1+4\lambda}\right)
\end{equation}
therefore:
\begin{equation}
F_L^{\rm FF,IC}(x,Q^2) = F_2^{\rm FF,IC}(x,Q^2) -2x F_1^{\rm FF,IC}(x,Q^2)  = \frac{4\lambda}{\sqrt{1+4\lambda}}B_c xc^+(\chi)
\end{equation}
...
expression evaluated in ``rescaled'' Bjorken variable $\chi$ defined
in eq.~(\ref{Rescaledx}). In fact, defining:
\begin{equation}\label{Rescaledx}
\eta=\frac{2Q^2}{\Sigma_{+-}+\Delta}\,,
\end{equation}
one can write:
\begin{equation}\label{Rescalingx}
\chi = \frac{x}{\eta}\,.
\end{equation}
One can also write that:
\begin{equation}\label{MassiveLOPDF}
\chi c(\chi) = \int_\chi^1\frac{dy}{y}y\delta(1-y)\frac{\chi}{y}c\left(\frac{\chi}{y}\right) = \int_\chi^1\frac{dy}{y}\frac{\chi}{y}\delta\left(1-\frac{\chi}{y}\right)yc(y)\,.
\end{equation}
Now we can use the usual interpolation formula to expand $yc(y)$:
\begin{equation}\label{interpolationF}
yc(y) = \tilde{c}(y) = \sum_{\alpha=0}^{N_x} w_{\alpha}^{(k)}(y)
\tilde{c}(x_{\alpha})\,,
\end{equation}
where $w_{\alpha}^{(k)}$ are lagrange interpolation functions of
degree $k$. Plugging eq.~(\ref{interpolationF}) into
eq.~(\ref{MassiveLOPDF}) and also using eq.~(\ref{Rescalingx}), one
finds:
\begin{equation}
\chi c(\chi) = \tilde{c}(\chi) = \sum_{\alpha=0}^{N_x} \left[\int_\chi^1\frac{dy}{y}\frac{\chi}{y}\delta\left(1-\frac{\chi}{y}\right)w_{\alpha}^{(k)}(y)\right]\tilde{c}(x_{\alpha})= \sum_{\alpha=0}^{N_x} w_{\alpha}^{(k)}(\chi)
\tilde{c}(x_{\alpha})
\end{equation}
